...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.MissingRequiredArgument):
        embed = nextcord.Embed(title=f":x: Command is missing required arguments.", description=f"```[{str(error)}]```",
                               color=lunablue)
        embed.set_footer(text=f"{bot.user.name} ✦ Created by andzn", icon_url="https://i.ibb.co/yBXMVKG/icon.png")
        await ctx.send(embed=embed)
    ...
    if isinstance(error, commands.BotMissingPermissions):
        embed = nextcord.Embed(title=f":x: I am missing permissions to do that", description=f"```[{str(error)}]```",
                               color=lunablue)
        embed.set_footer(text=f"{bot.user.name} ✦ Created by andzn", icon_url="https://i.ibb.co/yBXMVKG/icon.png")
        await ctx.send(embed=embed)
    ...
    if isinstance(error, commands.CommandOnCooldown):
        embed = nextcord.Embed(title=f":clock1: Command on cooldown, please wait", description=f"```[{str(error)}]```",
                               color=lunablue)
        embed.set_footer(text=f"{bot.user.name} ✦ Created by andzn", icon_url="https://i.ibb.co/yBXMVKG/icon.png")
        await ctx.send(embed=embed)
